...
// NOTE: mfaEnrollBtn / mfaVerifyBtn / mfaUnenrollBtn / enableBrowserNotifBtn all live
// inside js/views/settings.js, which only exists in the DOM after the router mounts
// the Settings route. A top-level `document.getElementById(...).addEventListener(...)`
// here would silently never fire (element doesn't exist yet at module-load time) — the
// working versions are the delegated `pageRoot.addEventListener('click', ...)` handlers
// further down, which re-check e.target on every click so they work no matter when the
// view was mounted or re-rendered.
...
document.body.addEventListener('click', async (e) => {
  ...
  if (e.target.id === 'enableBrowserNotifBtn' || e.target.closest('#enableBrowserNotifBtn')) {
    ...
    if (perm === 'granted') {
      ...
      // Immediate confirmation notification — the real bill-reminder notification
      // (maybeNotifyBills, below) only fires when a bill is due within 1 day, which a
      // fresh/demo account usually doesn't have. Without this, there's no way to tell
      // whether the feature actually works.
      try {
        new Notification('Cashora', {
          body: "You're all set. We'll notify you here when a bill is due soon.",
          icon: '/assets/icons/icon-192.webp',
        });
      } catch (_) {}
      maybeNotifyBills();
