...
└── assets/
    ├── icons/                # PWA icons (icon-192.webp, icon-512.webp, apple-touch-icon.webp)
    ├── images/
    │   └── og-image.webp    # Social share preview (Open Graph / Twitter card)
    └── fonts/
```
...
6. **Allow the reset-password redirect**: in Supabase → Authentication → URL Configuration, set **Site URL** to your deployed URL (e.g. `https://cashora.vercel.app`) and add `https://cashora.vercel.app/reset-password.html` under **Redirect URLs**. Without this, the "Forgot password?" email link won't be able to land on `reset-password.html`.
...
- [x] Phase 14 — Bug fixes from real device testing (see below)

## Phase 14 — Real-device bug fixes
Found by actually testing on a phone, not just code review. Each was a genuine bug, not a style nitpick:

- **Service worker offline gap**: only `views/networth.js` was in the install-time precache list — the other 7 route files (dashboard, transactions, budget, goals, insights, analytics, settings) weren't, so a route never visited online before could fail to load offline. All 8 are now precached; `tests/smoke.mjs` asserts this stays true for any future view added.
- **`config.local.js` didn't actually do anything**: the file/pattern was documented but never imported anywhere, so overriding Supabase keys locally silently had no effect. `js/config.js` now uses a top-level `await import('./config.local.js')` with a try/catch fallback, so the override genuinely works (tested both with and without the file present) while still working with zero setup out of the box.
- **Dead event listeners**: 5 buttons (Enable notifications, 3 MFA actions, Add category) each had a `document.getElementById(id)?.addEventListener(...)` at module scope in `app.js` — but those elements live inside `js/views/settings.js`, which doesn't exist in the DOM until the router mounts it, so these listeners silently never bound. A second, correct delegated (`pageRoot.addEventListener('click', ...)`) handler happened to also exist for the same buttons and was doing the real work, so nothing was actually broken for the user — but the dead copies were removed to avoid someone editing the wrong one. `tests/smoke.mjs` now has a check that fails the build if this pattern reappears.
- **Browser notifications had no feedback**: `maybeNotifyBills()` only shows a notification when a bill is due within 1 day — fresh/demo accounts never have one, so clicking "Enable notifications" looked like it did nothing. It now also fires an immediate confirmation notification on enable, independent of whether any bill is actually due.
- **Password reset used a racy check**: `reset-password.html` called `isAuthenticated()` once immediately on load — a known Supabase timing issue, since `detectSessionInUrl` may not have finished parsing the recovery link yet. Replaced with `supabase.auth.onAuthStateChange` (reacts to the actual `PASSWORD_RECOVERY` event) plus explicit `exchangeCodeForSession` handling for Supabase projects that issue `?code=` style links instead of `#access_token=` hash links.
- **Sign-in page footer floated next to the card on mobile**: `auth.html`'s `<body>` had `display: flex` with no `flex-direction`, defaulting to `row` — the "Built by KRIXORA" credit (a sibling of the card, not nested inside it) sat to the right of the card instead of below it. Fixed with `flex-direction: column`.
- **Password visibility toggle wasn't keyboard-accessible**: it was a bare `<svg onclick>`, untabbable. Now a real `<button>` with `aria-label`/`aria-pressed`, and every auth input has an explicit `aria-label` (not just a placeholder).
- **"Built by KRIXORA" credit was missing the "Built by" text** in all 4 places it appears (sidebar, auth, reset-password, landing footer) — showed just the icon + wordmark.
...
- [ ] On a phone browser, "Add to Home Screen" shows the Cashora icon and opens standalone (no browser address bar)
...
- [ ] Settings → Enable browser notifications → grant permission → a confirmation notification appears immediately (doesn't require an actual bill to be due)
- [ ] On `auth.html`, the password eye-icon toggle is reachable and operable with Tab + Enter/Space, not just a mouse click
- [ ] On a phone-width screen, "Built by KRIXORA" sits centered below the sign-in card, not floating beside it
- [ ] Click "Forgot password?", follow the email link — the reset page recognizes the link and shows the new-password form (not "invalid or expired") for a freshly-requested link
...
- **Secrets hygiene**: real keys removed from source. Put credentials in `js/config.js` or gitignored `js/config.local.js` (copy `js/config.local.example.js`) — this override is actually wired up (top-level `await import`), not just documented.
...
- **Smoke tests**: `node tests/smoke.mjs` — 13 checks (no npm required).
